...
CUDA_VISIBLE_DEVICES=${gpu_index} python ${NEMO_CODE_ROOT}/nemo/dnn/test_video.py --data_dir ${NEMO_DATA_ROOT} --content ${content}${index} --lr_video_name ${input_resolution}p_${bitrate}kbps_s0_d300.webm --hr_video_name 2160p_12000kbps_s0_d300.webm --num_blocks ${num_blocks} --num_filters ${num_filters} --load_on_memory --output_height ${output_resolution} --train_type train_video
